			...
			{isLoading ? (
				...
				<div>
					<div>
						...
						<div className={`block sm:hidden w-full h-[350px] rounded-[20px] flex items-end justify-center`} style={{ backgroundColor: bgColor }}>
							<div className="overflow-hidden rounded-[20px]">
								<img src={course.images.cardImage} alt="yoga-card" />
							</div>
						</div>

						{/* Если ширина экрана от 640px, то отображать этот div */}
						<div className={`hidden sm:flex w-full h-[200px] sm:h-[310px] justify-between rounded-[20px] mt-[40px] sm:mt-[60px]`} style={{ backgroundColor: bgColor }}>
							<p className="text-white text-[45px] md:text-[60px] font-bold p-[30px] md:p-[40px]">{course.nameRU}</p>
							<div className="overflow-hidden relative rounded-[20px]">
								<img src={course.images.courseImage} alt="yoga-card" className={ id === "ab1c3f" ? "h-[120%] sm:mt-[-80px]" : id === "q02a6i" || "fi67sm" ? "" : "md:mr-[70px]"} />
							</div>
						</div>
					</div>
					...
					<div className="mt-[40px] sm:mt-[60px]">
						...
						<div className="flex flex-col xl:flex-row mt-[20px] sm:mt-[40px] gap-[17px] justify-between">
							{course.fitting.map((fitting: any, index: number) => (
								<div key={index} className="w-full xl:w-[368px] h-[141px] md:h-[110px] xl:h-[160px] pl-[15px] pr-[15px] flex bg-gradient-to-r from-[#151720] to-[#1E212E] items-center rounded-[20px]">
						...
						<div className="bg-[#BCEC30] lg:grid lg:grid-cols-3 grid-rows-2 gap-4 p-[20px] md:p-[50px] w-full h-auto lg:h-[146px] lg:p-[30px] rounded-[20px] items-center">
							{course.directions.map((directions: any, index: number) =>
								<p key={index} className="text-[18px] sm:text-[24px] md:text-[28px] text-left">✦ {directions}</p>
							)}
						</div>
